...
extension NetworkToolProtocol {
    ...
    static func loadNewsGroupId(articleURL: String, completionHandler:@escaping (_ idStr: String) -> ()) {
        
        Alamofire.request(articleURL).responseString { (response) in
            guard response.result.isSuccess else { return }
            if let value = response.result.value {
                var startIndex = value.range(of: "thread_id")!.upperBound
                var endIndex = value.range(of: "user_id")!.lowerBound
                var range = Range(uncheckedBounds: (lower: startIndex, upper: endIndex))
                var content:String = String(value[range])
                content =  content.sub(fromIndex: 3).substring(toIndex: 16)
                completionHandler(content)
            }
        }
    }
    static func loadNewsDetail(articleURL: String, completionHandler:@escaping (_ news: NewsModel) -> ()) {
        ...
        Alamofire.request(articleURL).responseString { (response) in
            ...
            if let value = response.result.value {
                ...
                var content:String = String(value[range])
                content =  content.sub(fromIndex: 3).substring(toIndex: 16)
                let url = "https://www.toutiao.com/api/comment/list/?offset=0&count=10"
                let params = ["group_id": content,
                              "item_id": content] as [String: Any]
                Alamofire.request(url, parameters: params).responseJSON { (response) in
                    // 网络错误的提示信息
                    guard response.result.isSuccess else { return }
                    if let value = response.result.value {
    ...
    static func loadCommenNewsDetail(articleURL: String, completionHandler:@escaping (_ ere: String, _ images: [NewsDetailImage], _ abstracts: [String])->()) {
    ...
    private static func trim(content: String) -> String {
        var s = String()
        s = content.replacingOccurrences(of: "\"", with: "")
    ...
    static func updateWeatherInfo(latitude: CLLocationDegrees, longitude: CLLocationDegrees ,completionHandler: @escaping (_ dongtais: Any) -> ()){
        ...
        Alamofire.request(url, parameters: params).responseJSON { (response) in
            // 网络错误的提示信息
            guard response.result.isSuccess else { completionHandler( "no_message"); return }
            if let value = response.result.value {
